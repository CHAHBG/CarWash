import { Client, Databases, Query } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('6905ffc70017b29b34c7')
    .setKey('standard_');

const databases = new Databases(client);
const databaseId = '69063306000938349d80';
const menuCollectionId = 'menu';

async function checkGrillades() {
    try {
        console.log('🍗 Vérification des produits "poulet-grille"...\n');
        
        const response = await databases.listDocuments(
            databaseId,
            menuCollectionId,
            [Query.equal('category', 'poulet-grille')]
        );
        
        console.log(`Trouvé ${response.documents.length} produits dans "poulet-grille":\n`);
        
        response.documents.forEach(doc => {
            console.log(`- ${doc.name}`);
            console.log(`  Prix: ${doc.price} FCFA`);
            console.log(`  Image: ${doc.image ? '✅' : '❌'}\n`);
        });
        
    } catch (error) {
        console.error('❌ Erreur:', error);
    }
}

checkGrillades();
