import { Client, Databases, Query } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('6905ffc70017b29b34c7')
    .setKey('standard_');

const databases = new Databases(client);

const databaseId = '69063306000938349d80';
const menuCollectionId = 'menu';

(async () => {
  const response = await databases.listDocuments(databaseId, menuCollectionId, [Query.limit(100)]);
  const categories = new Map();
  response.documents.forEach(doc => {
    const cat = doc.category || 'undefined';
    if(!categories.has(cat)) categories.set(cat, []);
    categories.get(cat).push(doc.name);
  });

  console.log('Categories comptées:', [...categories.keys()]);
  console.log('Résumé:');
  categories.forEach((items, cat) => {
    console.log(`- ${cat}: ${items.length} items`);
  });
})();
