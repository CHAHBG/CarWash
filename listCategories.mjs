import { Client, Databases, Query } from 'node-appwrite';

const client = new Client()
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('6905ffc70017b29b34c7')
    .setKey('standard_');

const databases = new Databases(client);

const databaseId = '69063306000938349d80';
const categoriesCollectionId = 'categories';

(async () => {
  const response = await databases.listDocuments(databaseId, categoriesCollectionId, [Query.limit(100)]);
  console.log('Categories:', response.documents.map(doc => ({ id: doc.$id, name: doc.name, slug: doc.slug })));
})();
